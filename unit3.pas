...
//__DAMN_SON_THAT'S_SOME_FINE_QUICKSORT_________
procedure sortList(left, right:Integer);
var i, pivot, fLeft, fRight : Integer;
  comparison : Unit1.r_highscore;
begin
  if left < right then {mind. 2 Elemente}
begin {Zerlegung vorbereiten}
  pivot := (left + right)div 2;
  comparison := highscoresList[pivot];
  fLeft := left;
  fRight := right;
  while fLeft <= fRight do {noch nicht fertig zerlegt?}
  begin
    while highscoresList[fLeft].points < comparison.points do {linkes Element suchen}
    fLeft := fLeft + 1;
    while highscoresList[fRight].points > comparison.points do {rechtes Element suchen}
    fRight := fRight - 1;
    if fLeft <= fRight then
      begin
        swap(fLeft,fRight); {Elemente tauschen}
        fLeft := fLeft + 1;
        fRight := fRight - 1;
      end;
  end;
  sortList(left,fRight); {li. und re. Teilfeld zerlegen}
...
begin
  for i := 1 to 10 do begin
   highscoresList[i].points := 0;
   highscoresList[i].name := 'leer';
  end;
 Rewrite(saveFile);
